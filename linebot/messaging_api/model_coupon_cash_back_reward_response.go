...
func (cr *CouponCashBackRewardResponse) UnmarshalJSON(data []byte) error {
	...
	if raw["priceInfo"] != nil {
		...
		if rawpriceInfo, ok := raw["priceInfo"]; ok && rawpriceInfo != nil {
			PriceInfo, err := UnmarshalCashBackPriceInfoResponse(rawpriceInfo)
			if err != nil {
				return fmt.Errorf("JSON parse error in CashBackPriceInfoResponse(discriminator): %w", err)
			}
...
func (r *CouponCashBackRewardResponse) MarshalJSON() ([]byte, error) {
	...
	r.PriceInfo = setDiscriminatorPropertyCashBackPriceInfoResponse(r.PriceInfo)

	type Alias CouponCashBackRewardResponse
